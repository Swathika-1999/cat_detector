...
def define_model():
    # load model
    ml_model = VGG16(include_top=False, input_shape=(224, 224, 3))
    # mark loaded layers as not trainable
    for layer in ml_model.layers:
        layer.trainable = False
    # add new classifier layers
    flat1 = Flatten()(ml_model.layers[-1].output)
    class1 = Dense(128, activation='relu', kernel_initializer='he_uniform')(flat1)
    output = Dense(1, activation='sigmoid')(class1)
    # define new model
    model = Model(inputs=ml_model.inputs, outputs=output)
    # compile model
...
def run_test_harness():
    ...
    #final_model
    train_it = datagen.flow_from_directory('D:\\UDAY\\ML_API\\finalize_dogs_vs_cats\\',
                                           class_mode='binary', batch_size=64, target_size=(224, 224))
